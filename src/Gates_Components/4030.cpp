...
namespace nts {
    Component4030::Component4030(const std::string& name) 
        : AComponent(name)
    {
        gates[0] = std::make_shared<XorComponent>(name + "_XOR1");
        gates[1] = std::make_shared<XorComponent>(name + "_XOR2");
        gates[2] = std::make_shared<XorComponent>(name + "_XOR3");
        gates[3] = std::make_shared<XorComponent>(name + "_XOR4");
    }
    ...
    Tristate Component4030::compute(std::size_t pin) const {
        switch (pin) {
    ...
    void Component4030::setLink(std::size_t pin, IComponent& other, std::size_t otherPin) {
        switch (pin) {
            case 1: case 2:
    ...
    std::string Component4030::getType() const {
        return "4030";
    }
}
